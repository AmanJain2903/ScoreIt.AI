...
- If a user registers with simple email rather than Google OAuth, user is sent an email verification link.
- User can only log in once he verifies his email using that link.
- It prevents scammers from entering the system and only allows legitimate users to use the software.
