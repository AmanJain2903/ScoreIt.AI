...
def test_certification_matchmaker_end_to_end(certification_matchmaker):
    ...
    required_columns = ['ResumeCertifications', 'JobCertifications']
    for col in required_columns:
        if col not in dataset.columns:
            raise ValueError(f"Column '{col}' not found in the dataset.")
    randomIndex = np.random.randint(0, len(dataset))
    resume_certification = dataset['ResumeCertifications'].iloc[randomIndex]
    job_certification = dataset['JobCertifications'].iloc[randomIndex]
